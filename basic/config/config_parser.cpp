...
inline std::string add_quotes_if_needed(const std::string &str) {
  ...
  for (char c : str) {
    if ((std::isspace(static_cast<unsigned char>(c)) != 0) || c == '"' || c == '\'' || c == '\\' || c == '\r' ||
        c == '\n' || c == '\t' || c == '\t') {
      needs_quotes = true;
      break;
    }
...
IniParser::IniParser() {
  options().set_option("fmt_boolean_true", "true");
  options().set_option("fmt_boolean_false", "false");
  options().set_option("fmt_double_precision", 6);
}
bool IniParser::parse(const std::string &data, ConfigNode &root) {
  ...
  while (std::getline(stream, line)) {
    ...
    if (line.front() == '[' && line.back() == ']') {
      ...
      root[current_section] = ConfigNode();
      continue;
    }
    //
    ...
    std::string value = util::trim_quotes(util::unescape( util::trim(line.substr(delimiter_pos + 1))));
    //printf("%s\n", value.c_str());
...
std::string IniParser::dump(const ConfigNode &root) {
  static std::string enter_line = options().get_option<std::string>("enter_line", "\n");
  static std::string key_value_delimiter = options().get_option<std::string>("ini_key_value_delimiter", "=");
  if (root.is_object()) {
    std::string result;
...
std::string ConfParser::dump(const ConfigNode &root) {
  static std::string enter_line = options().get_option<std::string>("enter_line", "\n");
  static std::string key_value_delimiter = options().get_option<std::string>("conf_key_value_delimiter", "=");
  if (root.is_object()) {
    std::string result;
