...
class buffer_t {
  ...
  buffer_t(const buffer_t &rhs) : cap_(rhs.cap_), size_(rhs.size_), data_((char *)malloc(rhs.cap_)) {
    memcpy(data_, rhs.data_, rhs.size_);
  }

  buffer_t(buffer_t &&rhs) noexcept : cap_(rhs.cap_), size_(rhs.size_), data_(rhs.data_) {
    rhs.cap_ = 0;
    rhs.size_ = 0;
    rhs.data_ = nullptr;
  }
  ...
  size_type resize(size_type len) const {
    if (len > cap_ - size_) {
      return cap_;
    }
    //
  ...
  [[nodiscard]] size_type data_size() const {
