...
class lock_free_array {
  ...
  __attribute__((aligned(64))) size_t write_index = 0;
  __attribute__((aligned(64))) size_t read_index = 0;
  __attribute__((aligned(64))) size_t max_num_read_index = 0;
  TYPE queue_[N];

  public:
  static_assert((N & (N - 1)), "the size must be a power of 2 !!!");
  lock_free_array() = default;
  ~lock_free_array() {
  }

  lock_free_array(const lock_free_array<TYPE, N> &arr) = delete;
  lock_free_array<TYPE, N> operator=(lock_free_array<TYPE, N> &rhs) = delete;
  lock_free_array(lock_free_array<TYPE, N> &&arr) = delete;
  lock_free_array<TYPE, N> operator=(lock_free_array<TYPE, N> &&rhs) = delete;
